...
Default requiqrement are two input files, one containing the numbers for the the model feature data
...
Requirement: Input csv file must include one column labeled "log_crime_rate" which holds the log crime rate data
"""
...
shape_filename = "abs/shape_files/SA2_2011_AUST.shp"
shape_region = 'New South Wales' # set to None if no particular region exctracted and all shapedata should be included
...
simulate = True # Creates simulated data with 2dim spatial coordinates and 2 features and then runs code on this data.
calc_center = True # Calculates centroids from polygons and converts from Lat/Lng to cartesian with center coord below.
...
create_maps = True # creates interactive html maps of crime results, not included for simulated data.
# GP setup
...
nburn = 100 # Number of iterations, recommended at least 20% of niter, check sampler chains for convergence.
